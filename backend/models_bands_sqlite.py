"""
Simplified models for SQLite development - replacing JSONB with TEXT/JSON
"""
...
class Band(db.Model):
    ...
    genres = db.Column(db.Text, nullable=True)  # JSON array
    ...
    photo_gallery_urls = db.Column(db.Text, nullable=True)  # JSON array
...
class Venue(db.Model):
    ...
    # JSON fields as TEXT
    primary_genres = db.Column(db.Text, nullable=True)  # JSON array
    contact_email = db.Column(db.String(255), nullable=True)
    contact_phone = db.Column(db.String(20), nullable=True)
    website = db.Column(db.String(255), nullable=True)
    social_links = db.Column(db.Text, nullable=True)  # JSON object
    ...
    def to_dict(self, include_stats=False):
        return {
            ...
            'profile_image_url': self.profile_image_url,
            'photo_gallery_urls': json.loads(self.photo_gallery_urls) if self.photo_gallery_urls else [],
            'is_active': self.is_active,
            'is_verified': self.is_verified,
            'created_at': self.created_at.isoformat() if self.created_at else None,
            'updated_at': self.updated_at.isoformat() if self.updated_at else None
...
# Simplified models for core functionality
class Gig(db.Model):
    __tablename__ = 'gigs'
    ...
    def to_dict(self):
        return {
            ...
            'doors_time': self.doors_time.isoformat() if self.doors_time else None,
...
# Add minimal other tables needed for the API
...
class BandFollower(db.Model):
    ...
    # Store notification preferences as simple JSON text
    notification_preferences = db.Column(db.Text, default='{"new_gigs": true, "new_releases": false, "band_updates": true}')
...
class Setlist(db.Model):
    ...
    # Store songs as JSON text
    songs = db.Column(db.Text, nullable=False)  # JSON array of song objects
    notes = db.Column(db.Text, nullable=True)
    encore_songs = db.Column(db.Text, nullable=True)  # JSON array
...
class ReviewVote(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    review_id = db.Column(db.Integer, nullable=False)  # Can be band_review or venue_review
    review_type = db.Column(db.String(20), nullable=False)  # 'band' or 'venue'
    user_id = db.Column(db.String(36), db.ForeignKey('users.id'), nullable=False)
